...
CREATE TABLE tool_invocation (
    ...
    task_id             VARCHAR(128)    DEFAULT NULL COMMENT '任务标识',
    ...
    duration_ms         BIGINT UNSIGNED DEFAULT NULL COMMENT '总耗时（毫秒）',
    ...
    result_artifact_uri VARCHAR(1024)   DEFAULT NULL COMMENT '完整结果归档地址（可选，如对象存储）',
    ...
    KEY idx_tool_invocation_task_id (task_id),
    KEY idx_tool_invocation_tool_started_at (tool_name, started_at),
    KEY idx_tool_invocation_status_started_at (status, started_at)
) ENGINE = InnoDB
...
CREATE TABLE tool_invocation_attempt (
    ...
    duration_ms      BIGINT UNSIGNED DEFAULT NULL COMMENT '本次尝试耗时（毫秒）',
    ...
    KEY idx_tool_invocation_attempt_status_started_at (status, started_at),
    CONSTRAINT fk_tool_invocation_attempt_invocation
